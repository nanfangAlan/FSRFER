...
class Generator(object):
    ...
    def __init__(self, FLAGS):
        ...
        self.feature_channel = FLAGS.feature_channel
    ...
    def _conv_RRDB(self, x, out_channel, num=None, activate=True):
        with tf.variable_scope('block_{0}'.format(num)):
            ...
            x = tf.layers.BatchNormalization(name='batch_norm')(x)
            if activate:
    ...
    def build(self, x):
        ...
        with tf.variable_scope('last_conv'):
            x = tf.layers.conv2d(x, self.n_filter, 8, 1, padding='same', kernel_initializer=self.init_kernel,
                                 name='conv_1')
            x = tf.nn.leaky_relu(x, alpha=0.2, name='leakyReLU')
            x = tf.layers.conv2d(x, self.feature_channel, 8, 1, padding='same', kernel_initializer=self.init_kernel,
                                 name='conv_2')
...
class SimpleGenerator(object):
    ...
    def _denseBlock(self, x, num=None):
        with tf.variable_scope('DenseBlock_sub{0}'.format(num)):
            x1 = self._conv_RRDB(x, self.inc_filter, 0)
            x2 = self._conv_RRDB(tf.concat([x, x1], axis=3), self.inc_filter, 1)
            x3 = self._conv_RRDB(tf.concat([x, x1, x2], axis=3), self.n_filter, 2,activate=False)

        return x3 * self.residual_scaling
    ...
    def build(self, x):
        ...
        with tf.variable_scope('last_conv'):
            x = tf.layers.conv2d(x, self.n_filter, 5, 1, padding='same', kernel_initializer=self.init_kernel,
                                 name='conv_1')
            x = tf.nn.leaky_relu(x, alpha=0.2, name='leakyReLU')
            x = tf.layers.conv2d(x, self.feature_channel, 5, 1, padding='same', kernel_initializer=self.init_kernel,
                                 name='conv_2')
...
class SimpleDiscriminator(object):
    ...
    def build(self, x):
        with tf.variable_scope('first_conv'):
            x = tf.layers.conv2d(x, self.n_filter, 5, 2, padding='same', use_bias=False,
                                 kernel_initializer=self.init_kernel, name='conv_1')
            x = tf.nn.leaky_relu(x, alpha=0.2, name='leakyReLU_1')
            x = tf.layers.conv2d(x, self.n_filter*2, 5, 2, padding='same', use_bias=False,
                                 kernel_initializer=self.init_kernel, name='conv_2')
            x = tf.nn.leaky_relu(x, alpha=0.2, name='leakyReLU_2')
            x = tf.layers.conv2d(x, self.n_filter*4, 3, 2, padding='same', use_bias=False,
                                 kernel_initializer=self.init_kernel, name='conv_3')
            x = tf.nn.leaky_relu(x, alpha=0.2, name='leakyReLU_3')
            x = tf.layers.conv2d(x, self.n_filter*8, 3, 1, padding='same', use_bias=False,
                                 kernel_initializer=self.init_kernel, name='conv_4')
            x = tf.nn.leaky_relu(x, alpha=0.2, name='leakyReLU_4')
...
class ACDiscriminator(object):
    """the definition of wgan Discriminator"""
    ...
    def build(self, x):
        ...
        with tf.variable_scope('full_connected'):
            x = tf.layers.flatten(x)
            x = tf.layers.dense(x, 2000, name='fully_connected_1')
            x = tf.nn.leaky_relu(x, alpha=0.2, name='leakyReLU_1')
            x = tf.layers.dense(x, 128, name='fully_connected_2')
            x = tf.nn.leaky_relu(x, alpha=0.2, name='leakyReLU_2')
            logits = tf.layers.dense(x, 7, name='fully_connected_3')
            x = tf.nn.leaky_relu(logits, alpha=0.2, name='leakyReLU_3')
            x = tf.layers.dense(x, 1, name='fully_connected_4')
