...
def inference(images, keep_probability=0.8, phase_train=False, bottleneck_layer_size=7, weight_decay=5e-2, reuse=None, batch_size=128):
    batch_norm_params={
        'decay':0.995,
        'epsilon':0.001,
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        return network(images, is_training=False,dropout_keep_prob=keep_probability,bottleneck_layer_size=bottleneck_layer_size, batch_size = batch_size)

def network(inputs,is_training=False,dropout_keep_prob=1,bottleneck_layer_size=7, reuse=tf.AUTO_REUSE, scope='DLP-CNN', batch_size=128):
    #dropout_keep_prob=1
    with tf.variable_scope(scope,'DLP-CNN',[inputs],reuse=reuse):
        with slim.arg_scope([slim.batch_norm,slim.dropout],is_training=is_training):
            with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d], stride=1, padding='SAME'):
                ...
                with tf.variable_scope('spdpooling1') as scope:
                    ...
                    reshaped = tf.reshape(net, [batch_size, shape[1]*shape[2], shape[3]])
                    # Cov Pooling Layer
            ...
            local9 = tf.reshape(local9,[shape[0],shape[1],shape[2],1])
            return local9

def inference_middle(images,keep_probability=0.8, phase_train=False, bottleneck_layer_size=7, weight_decay=5e-2, reuse=None, batch_size=128):
    batch_norm_params={
        'decay':0.995,
        'epsilon':0.001,
        'updates_collections':None,
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        normalizer_fn=slim.batch_norm,
        normalizer_params=batch_norm_params):
        return middle(images, is_training=phase_train,dropout_keep_prob=keep_probability,bottleneck_layer_size=bottleneck_layer_size, batch_size=batch_size)

def middle(inputs,is_training=False, dropout_keep_prob=1, bottleneck_layer_size=7, reuse=tf.AUTO_REUSE, scope='DLP-CNN',batch_size=128):
        with tf.variable_scope(scope, 'DLP-CNN', [inputs], reuse=reuse):
            with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
                with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d], stride=1, padding='SAME'):
                    net = tf.reshape(inputs,[batch_size,-1])
                    net = slim.fully_connected(net, 2000, activation_fn=None, scope='fc_1', reuse=False)
                    net = tf.nn.relu(net)
                    net_f = slim.fully_connected(net, 128, activation_fn=None, scope='fc_2', reuse=False)
                    net = tf.nn.relu(net_f)
                    net = slim.fully_connected(net, bottleneck_layer_size, activation_fn=None, scope='Bottleneck', reuse=False)
                    return net,net_f

def last(inputs,batch_size=None):
    inputs = tf.reshape(inputs, [batch_size, -1])
    logits = slim.fully_connected(inputs, 7, trainable=False, activation_fn=None,
                                       weights_initializer=tf.truncated_normal_initializer(stddev=0.1),
                                       weights_regularizer=slim.l2_regularizer(5e-2),
